...
taskname=$1
file="results_checkpoints/downstream_results/${taskname}_results.csv"

# First pass: Find the maximum value in column 7 for each unique value in the first column
awk -F',' 'NR > 1 && $4 != "nan                 " && $2 != "nan                 " {if (!($1 in max) || $4 > max[$1]) max[$1] = $4} END {for (key in max) print key "," max[key]}' "$file" > max_values.txt
# Second pass: Print the header and all lines where the value in the first column matches and column 7 is the maximum for that group
{
    head -n 1 "$file"
    awk -F',' 'NR == FNR {max[$1] = $2; next} NR > 1 && $4 == max[$1]' max_values.txt "$file"
} > "results_checkpoints/downstream_results/${taskname}_results_final_valid.csv"
