...
def main(cfg):
    ...
    checkpoint = torch.load('/p/scratch/hai_oneprot/bazarova1/checkpoints_pa/2025-01-31_145706/checkpoint_epoch20_valloss0.4820.pt', map_location=device)
...
#    print(new_state_dict.items(),"new_state_dict!!!!!!!!!!")
    ...
    val_dl = DataLoader(
        dataset=datamodule.datasets[f"{cfg.modality_data_name}_test"],
        batch_size=32,
        num_workers=datamodule.num_workers,
        pin_memory=datamodule.pin_memory,
        collate_fn=datamodule.datasets[f"{cfg.modality_data_name}_test"].text_collate_fn,
        drop_last=False,
        shuffle=False,
        sampler=DistributedSampler(datamodule.datasets[f"{cfg.modality_data_name}_test"], 
                              shuffle=False, 
                              rank=rank, 
    ...
    # val_dl_struct_graph = DataLoader(
    #     dataset=datamodule.datasets["struct_graph_test"],
    #     batch_size=32,
    #     num_workers=datamodule.num_workers,
    #     pin_memory=datamodule.pin_memory,
    #     collate_fn=datamodule.datasets["struct_graph_test"].text_collate_fn,
    #     drop_last=False,
    #     shuffle=False,
    #     sampler=DistributedSampler(datamodule.datasets["struct_graph_test"], 
    #                           shuffle=False, 
    #                           rank=rank, 
    #                           num_replicas=world_size)
    #     )

    # val_dl_struct_token = DataLoader(
    #     dataset=datamodule.datasets["struct_graph_val"],
    #     batch_size=32,
    #     num_workers=datamodule.num_workers,
    #     pin_memory=datamodule.pin_memory,
    #     collate_fn=datamodule.datasets["struct_token_val"].text_collate_fn,
    #     drop_last=False,
    #     shuffle=False,
    #     sampler=DistributedSampler(datamodule.datasets["struct_token_val"], 
    #                           shuffle=False, 
    #                           rank=rank, 
    #                           num_replicas=world_size)
    #     )
    ...
    # val_dl_pocket = DataLoader(
    #     dataset=datamodule.datasets["pocket_val"],
    #     batch_size=32,
    #     num_workers=datamodule.num_workers,
    #     pin_memory=datamodule.pin_memory,
    #     collate_fn=datamodule.datasets["pocket_val"].text_collate_fn,
    #     drop_last=False,
    #     shuffle=False,
    #     sampler=DistributedSampler(datamodule.datasets["pocket_val"], 
    #                           shuffle=False, 
    #                           rank=rank, 
    #                           num_replicas=world_size)
    #     )
    ...
    def get_validation_sample(val_dl, device):
        ...
        modality_input, text_input, seq = next(val_iter)
        modality_input = modality_input.to(device)
        return modality_input, text_input, seq

    if rank == 0:
        print("Generation phase:")

        val_modality_input, val_text_input, seq = get_validation_sample(val_dl, device)
        #print(val_modality_input[0].shape,len(val_text_input[0]),len(data[0])," input!!!!!!!!!!!!!")
        ...
        print(input_embeds.shape," input embeds shape before!!!!!!!!!!!!")
        ...
        # for i in range(0,32):
        #     output_text = tokenizer_local.decode(outputs[i][:-2], skip_special_tokens=True)
        #     print("Output:", output_text)
        #     print(f"Ground truth: {val_text_input[i]}",data[i])
    ...
    # print("\nStructure Output:", structure_text)
        ...
        # Get random row
        #random_row = df.head(30) #df.sample(n=1000)
        ind=[0,321,323,569,650,886,962,1459,2046,2085,2145,2322,2375,2707,2760,3112,3412]
        random_row=df.iloc[ind]
    ...
    # modality_embs = encode_modality(modality_input, modality_encoder, modality_proj).to(device)
        ...
        #text_embedding = text_tokenizer(val_text_input, return_tensors="pt",max_length=512,truncation=True,padding=True)["input_ids"].to(device)
        #print(val_text_input[0]," val_text_input!!!!!!!!!!")
        #text_embedding=tokenizer_local(val_text_input[0], padding=True,truncation=True,add_special_tokens=False).input_ids
        ...
        with h5py.File('/p/scratch/hai_oneprot/merdivan1/pretrain_dataset/50ss/seqstruc.h5', 'r') as file:
            ...
            sequences = [file[structure]['structure']['0']['A']['residues']['seq1'][()].decode('utf-8') 
                         for structure in random_row['structure']]
            input_struct_graph = [
                protein_to_graph(structure, '/p/scratch/hai_oneprot/merdivan1/pretrain_dataset/50ss/seqstruc.h5', 'non_pdb', 'A', pockets=False)
            for structure in random_row['structure']
                ]
            input_struct_graph = Batch.from_data_list(input_struct_graph).to(device)

        seq_embedding = seq_tokenizer(sequences, return_tensors="pt",padding=True, truncation=True)["input_ids"].to(device)
        print(seq_embedding.shape," seq_embedding shape!!!!!!!!!!")
        ...
        for emb in generation_inputs.keys():
            if '_embeds' in emb:
                ...
                #print(input_embeds," input embeds!!!!!!!!!!")
                print(input_embeds.shape," input embeds shape!!!!!!!!!!!!")
                stopping_criteria = StoppingCriteriaList([StoppingCriteriaSub(stops=[2277], encounters=1)])
                outputs = model.module.generate(
                        inputs_embeds=input_embeds,
                        max_new_tokens=generation_inputs['max_tgt_len'],
                ...
                for i in range(0,len(ind)):
                    output_text = tokenizer_local.decode(outputs[i][:-2], skip_special_tokens=True)
                    print("Output:", output_text)
                    print(f"Ground truth: {random_row['func_text'][ind[i]]}")
