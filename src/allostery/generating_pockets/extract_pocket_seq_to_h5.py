...
def write_to_h5(output_h5, identifier, atom_amino_id, atom_names, atom_pos, amino_types,
    ...
    Also stores the full-chain sequence at the identifier root so the embedding
    script can retrieve it offline without any network calls.
    
    Structure:
    identifier/
        full_sequence: full amino acid sequence of the chain (stored here for offline use)
        structure/
            0/
                chain/
                    residues/
                        seq1: pocket amino acid sequence (subset)
                    polypeptide/
    ...
    with h5py.File(output_h5, 'a') as f:
        ...
        # This is the complete chain sequence BEFORE count_cut2 clips
        # it to the pocket — used by the embedding script to get the
        # sequence input for the sequence encoder without network access.
        # -----------------------------------------------------------
        ...
        # Store pocket sequence as bytes (HDF5 standard)
...
def process_annotation_to_h5(annotation, output_h5, count=100):
    ...
    # amino_types here is the FULL chain sequence — save it before count_cut2 clips it
    ...
    # Write to H5 file — pass both the pocket data AND the full-chain sequence
...
annotations_file = '/p/data1/profound_data/CDPPILBP/ippidb-pdb-analyses-042023-zenodo/PL_part8_20230317_matrix_liganded_allosteric.csv'
process_annotations_batch(annotations_file, 'binding_pockets_seq_allosteric_competitive.h5', count=100)