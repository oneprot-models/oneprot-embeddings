...
class TextDatasetInstr(Dataset):
    def __init__(self, data_dir: str, split: str, max_length: int = 1024, text_max_length: int = 512, text_tokenizer: str = "allenai/scibert_scivocab_uncased", 
        ...
        json_pattern = f'{data_dir}/Protein-oriented_Instructions/*.json'
    ...
    def text_collate_fn(self, data):
        ...
        for seq_id in data:
            sequences.append(self.data[seq_id]['input'])
            text_prompts.append(self.data[seq_id]['instruction'])
            text_responses.append(self.data[seq_id]['output'])
        ...
        return sequence_input.long(), list(text_prompts), list(text_responses)