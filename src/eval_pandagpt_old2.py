...
@hydra.main(version_base="1.3", config_path="../configs", config_name="panda_gpt2.yaml")
...
#    modality_proj.load_state_dict(new_state_dict)
    ...
    if rank == 0:
        ...
        print("Modality dataloader length: ", len(modality_dl))
...
#    optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    ...
    for epoch in range(cfg.num_epochs):
        modality_dl.sampler.set_epoch(epoch)  # Set epoch for proper shuffling
        batch_losses = []
        batch_accuracies = []
    ...
    def get_validation_sample(val_dl, device):
        ...
        modality_input, text_input = next(val_iter)
        modality_input = modality_input.to(device)
