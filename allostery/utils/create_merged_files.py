...
POCKET variant (if ASD_pockets100 AND merged_pocket_binary both exist):
  1. Copy merged_pocket_binary → ASD_merged_pocket_binary
  2. Rename .pt files inside each split folder to:
       ASD_merged_pocket_binary_{split}_embeddings_labels.pt
...
SEQUENCE variant (if ASD_pockets_sequence100 AND merged_pocket_sequence_binary both exist):
  Same as above but with:
  - source:      merged_pocket_sequence_binary
  - destination: ASD_merged_pocket_sequence_binary
  - appended from: ASD_pockets_sequence100/{split}/ASD_pockets_sequence100_{split}_embeddings_labels.pt
  - new files named: ASD_merged_pocket_sequence_binary_{split}_embeddings_labels.pt
"""
...
def main(embeddings_root: str):
    ...
    for folder in subfolders:
        ...
        setup_merged_folder(
            folder=folder,
            src_name="merged_pocket_binary",
            dst_name="ASD_merged_pocket_binary",
            pockets100_name="ASD_pockets100",
            merged_prefix="ASD_merged_pocket_binary",
        ...
        setup_merged_folder(
            folder=folder,
            src_name="merged_pocket_sequence_binary",
            dst_name="ASD_merged_pocket_sequence_binary",
            pockets100_name="ASD_pocket_sequence100",
            merged_prefix="ASD_merged_pocket_sequence_binary",
