...
#SBATCH --time 0:20:00                                                            # pecify the running time; on develbooster it is limited to 2 hours
...
#SBATCH --output /p/scratch/hai_oneprot/bazarova1/slurms/esm_SaProt_collect%j_out.out  # specify the file for the slurm output; make sure to change it to your folder
#SBATCH --error /p/scratch/hai_oneprot/bazarova1/slurms/esm_SaProt_collect%j_err.out                                                          # do not change
...
#   -B /p/project1:/project1 \
#   /p/project1/hai_oneprot/merdivan1/singularity_docker_jupyter/singularity_docker_jupyter.sif \
#   bash -c "export CUDA_VISIBLE_DEVICES='0,1,2,3'; export WORLD_SIZE=$SLURM_NTASKS; export PYTHONPATH=''; export HYDRA_FULL_ERROR=1; source /p/project1/hai_oneprot/bazarova1/sc_venv_erinc2/venv/bin/activate; python /p/project1/hai_oneprot/aboelsoud1/oneprot-panda/src/collect_embeddings.py single_batch_mode=true"

#########From Alina 
srun --cpu-bind=none bash -c "export CUDA_VISIBLE_DEVICES=\"0,1,2,3\"; export WORLD_SIZE=$SLURM_NTASKS; export PYTHONPATH=\"\"; export HYDRA_FULL_ERROR=1; apptainer run --nv /p/project1/hai_oneprot/merdivan1/singularity_docker_jupyter/singularity_docker_jupyter.sif bash -c \"source /p/project1/hai_oneprot/bazarova1/sc_venv_erinc2/venv/bin/activate && python /p/project1/hai_oneprot/aboelsoud1/oneprot-panda/src/collect_embeddings.py single_batch_mode=true\"" 
